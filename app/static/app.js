// Hospital Assistant - Complete Single Shell Controller with RBAC
...
const demoData = {
  doctors: [
    { id: 1, name: "Ava Bennett", specialization: "Cardiology", department_id: 1, department: { name: "Cardiology" }, email: "ava.bennett@mercyharbor.example", phone: "+1 555 0101" },
    { id: 2, name: "Noah Williams", specialization: "Internal Medicine", department_id: 2, department: { name: "Internal Medicine" }, email: "noah.williams@mercyharbor.example", phone: "+1 555 0102" },
    { id: 3, name: "Mia Patel", specialization: "Pediatrics", department_id: 3, department: { name: "Pediatrics" }, email: "mia.patel@mercyharbor.example", phone: "+1 555 0103" },
  ],
  departments: [
...
function demoChatAnswer(question) {
  ...
  let answer = "Mercy Harbor Hospital information is available from the verified policy collection.";
  ...
  else if (/laboratory|lab report/.test(normalized)) { answer = "Yes. Selected laboratory reports can be viewed through the SunriseCare patient portal."; document = "Patient Portal"; }
...
async function demoApiFetch(endpoint, options = {}) {
  ...
  if (endpoint === "/auth/login" && method === "POST") {
    const isAdmin = body.email === "admin@admin.com";
    return demoResponse({ access_token: `demo-token-${isAdmin ? "admin" : "patient"}`, email: body.email || "patient@hospital.com", role: isAdmin ? "admin" : "patient", user_id: isAdmin ? 1 : 2 });
  }
...
function updateAuthDisplay() {
  ...
  if (authToken && currentUser) {
    ...
    if (authStatusIndicator) {
      if (role === "admin") {
        authStatusIndicator.textContent = `👑 Hospital Administrator: ${currentUser.email}`;
        authStatusIndicator.style.color = "var(--emergency-red)";
      } else if (role === "staff") {
        authStatusIndicator.textContent = `🩺 Hospital Staff: ${currentUser.email}`;
    ...
    if (authStatusIndicator) {
      authStatusIndicator.textContent = "Guest Mode (Sign In for appointments & portal access)";
...
const viewTitles = {
  assistant: "AI Knowledge Assistant (RAG)",
  appointments: "Appointment Scheduling & Management",
  directory: "Doctors & Clinical Departments Directory",
  patients: "Patient Medical Records (Admin/Staff Only)",
  knowledge: "Knowledge Documents & Ingestion (Admin/Staff Only)",
};

function switchView(viewName) {
  // Enforce RBAC guard on view switching
  if ((viewName === "patients" || viewName === "knowledge") && !isAdminOrStaff()) {
    showToast("Access Restricted: Hospital Staff or Administrator role required.", "error");
    return;
  }
  ...
  if (viewTitle) {
    viewTitle.textContent = viewTitles[viewName] || "Hospital Assistant";
  }
...
function renderSources(sources) {
  ...
  if (!sources || sources.length === 0) {
    sourcesList.innerHTML =
      '<div class="empty-state">No sources retrieved for this response.<br>Ask a specific hospital question.</div>';
    return;
  }
...
async function sendChatQuery(customQuery) {
  ...
  try {
    ...
    if (res.status === 401) {
      appendChatMessage("Hospital Assistant", "Authentication required. Please sign in to continue.");
      renderSources([]);
      return;
    }
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      appendChatMessage("Hospital Assistant", `Error: ${err.detail || "Unable to retrieve response from assistant."}`);
      renderSources([]);
      return;
    }

    const data = await res.json();
    appendChatMessage("Hospital Assistant", data.answer, data.emergency);
    renderSources(data.sources);

  } catch (err) {
    appendChatMessage("Hospital Assistant", `Connection error: ${err.message}. Ensure backend is running.`);
    renderSources([]);
  } finally {
  ...
  try {
    ...
    depts.forEach(d => {
      ...
      card.innerHTML = `
        ...
        <p>${escapeHtml(d.description || "Hospital specialized clinical department.")}</p>
  ...
  demoLoginPatientBtn.addEventListener("click", async () => {
    ...
    if (!ok) {
      try {
        ...
        await performLogin("patient@hospital.com", "patient123");
      } catch (err) {
