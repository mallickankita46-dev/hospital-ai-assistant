...
app = FastAPI(
    ...
    description=(
        "Hospital Knowledge and Appointment Assistant REST API with JWT authentication, "
        "role-based access control, departmental and appointment management, document ingestion, "
        "FAISS vector search, and grounded RAG chatbot."
    ),
    version="1.0.0",
    lifespan=lifespan,
    openapi_tags=[
        ...
        {"name": "chat", "description": "Hospital RAG assistant and safety guards"},
    ],
)
...
def root_message(request: Request, format: str | None = None):
    ...
    if format == "json" or ("application/json" in accept and "text/html" not in accept):
        return {
            "message": "Welcome to Hospital Knowledge and Appointment Assistant API",
            "status": "ok",
            "docs": "/docs",
            "health": "/health",
            "ui": "/ui/index.html",
